class ListingPhotosController < ApplicationController
    ...
    @listing_photo = @listing.listing_photos.build(listing_photo_params)
      if @listing_photo.save
          respond_to do |format|
              format.html { redirect_to host_listing_path(@listing), notice: "Listing photo successfully uploaded" }
          end
      else
  ...
  def destroy
      if @listing_photo.destroy
          respond_to do |format|
              format.html { redirect_to host_listing_path(@listing), notice: 'Listing photo deleted' }
          end
      end
  end
  ...
  private
      ...
      def listing_photo_params
        params.require(:listing_photo).permit(:photo, :caption)
      end
